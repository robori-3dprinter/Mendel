...
module vertex(with_foot=basefoot)
{
	translate ([0,0,vfvertex_height/2])difference()
	{
		union ()
		{
			if (with_foot)
			{
				dxf_linear_extrude(file = "vertex-body-fixed-foot.dxf",height=vfvertex_height,center=true);					}
			else
			{
		...
		if (with_foot)
		{
			dxf_linear_extrude(file = "vertex-body-hole-foot.dxf",height=2*vfvertex_height,center=true);
		}
		else
		{
			dxf_linear_extrude(file = "vertex-body-hole-qcad.dxf",height=2*vfvertex_height,center=true);
		}

		// Enlarge radius by 0.2mm
		translate(vertex_x_rod_1) cylinder(h=18,r=(m8_diameter/2 + 0.2),center=true);
		translate(vertex_x_rod_2) cylinder(h=18,r=(m8_diameter/2 + 0.2),center=true);

		translate([40,21,0]) xteardrop(m8_horizontal,62);
		translate([13.687,41.010,0]) rotate(a=60,v=[0,0,1]) xteardrop(m8_horizontal,62);
	}
}
...
module xteardrop(diameter,length)
{
	rotate(a=-90,v=[0,1,0]) rotate(a=-90,v=[0,0,1]) zteardrop(diameter,length);
}

module yteardrop(diameter,length)
{
	rotate(a=90,v=[1,0,0]) zteardrop(diameter,length);
}
...
module vertex_foot()
{
	difference ()
	{
		...
		if(drill_diag)
		{
			translate ([0,diag_y,0]) rotate([0,diagonal,0]) xteardrop(m8_horizontal,62);
			translate ([-15,diag_y,6]) rotate([0,diagonal,0]) cube([20,20,20],center=true);
			translate ([15,diag_y,-6]) rotate([0,diagonal,0]) cube([20,20,20],center=true);
		}
	}
}
